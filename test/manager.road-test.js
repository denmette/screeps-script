...
describe("manager.road", () => {
  ...
  it("constructor", () => {
    ...
    assert.equal(
      startsWith,
      RoadManager.toString().substring(0, startsWith.length),
    );
  });
  ...
  describe("#watchRoom", () => {
    it("lifecycle", () => {
      ...
      var array = watcher._createTileArray(
        watcher._fetchMemoryOfWatcher().currentTiles,
      );
      for (var x = 0; x < 50; x++) {
        for (var y = 0; y < 50; y++) {
          if (x == 12 && y == 34) {
      ...
      assert.equal(
        true,
        watcher._fetchMemoryOfWatcher().lastTiles === undefined,
      );
      ...
      var array = watcher._createTileArray(
        watcher._fetchMemoryOfWatcher().lastTiles,
      );
      for (var x = 0; x < 50; x++) {
        for (var y = 0; y < 50; y++) {
          if (x == 12 && y == 34) {
