...
describe("manager.room", () => {
  ...
  it("exists", () => {
    ...
    assert.equal(
      startsWith,
      RoomManager.toString().substring(0, startsWith.length),
    );
  });
  ...
  describe("constants", () => {
    var testConstant = function (
      ...
      assert.notEqual(
        true,
        constant.character == true,
        "Could not find character for " + JSON.stringify(constant),
      );
      assert.notEqual(
        undefined,
        constant.isEqual,
        "Could not find isEqual() for " +
          constant.character +
          " (" +
      ...
      var structures = acceptedStructures.filter((structure) =>
        constant.isEqual(structure),
      );
      assert.equal(
        1,
        structures.length,
        "Could not find accepted structure for " +
          constant.character +
          " (" +
          JSON.stringify(constant) +
          ")",
      ...
      var structures = notExpectedStructures.filter((structure) =>
        constant.isEqual(structure),
      );
      assert.equal(
        0,
        structures.length,
        "Found not accepted structure for " +
          constant.character +
          " (" +
          JSON.stringify(constant) +
          "): " +
    ...
    it("TILES_TERRAIN", () => {
      RoomManager.TILES_TERRAIN.forEach((constant) =>
        testConstant(
          constant,
          [
            // acceptedStructures
    ...
    it("TILES_PRESET", () => {
      RoomManager.TILES_PRESET.forEach((constant) =>
        testConstant(
          constant,
          [
            // acceptedStructures
    ...
    it("TILES_TO_BE_BUILD", () => {
      RoomManager.TILES_TO_BE_BUILD.forEach((constant) =>
        testConstant(
          constant,
          [
            // acceptedStructures
    ...
    it("TILES_ENDLESS_STRUCTURES", () => {
      RoomManager.TILES_ENDLESS_STRUCTURES.forEach((constant) =>
        testConstant(
          constant,
          [
            // acceptedStructures
  ...
  describe("#fetchTileAt", () => {
    ...
    var testFetchTileAt = function (stuff, expectedCharacter) {
      ...
      assert.equal(
        expectedCharacter,
        character,
        "Could not get character from " + JSON.stringify(stuff),
      );
    };
    it("plain", () => {
      testFetchTileAt(
  ...
  it("#initArray", () => {
    ...
    var room = new Room();
    room.lookAt = (x, y) =>
      y == startY && x < allTiles.length ? [allTiles[x].stuff] : [];
    ...
    for (var i = 0; i < allTiles.length; i++) {
      assert.equal(
        allTiles[i].expectedCharacter,
        roomManager._array.get(i, startY),
      );
    }
  });
  ...
  it("#initArrayIfNecessary", () => {
    ...
    roomManager._initArray = () => {
      assert.fail("_initArray() should not have been called again!");
    };
    roomManager._initArrayIfNecessary();
    roomManager._initArrayIfNecessary();
    roomManager._initArrayIfNecessary();
  ...
  describe("#fetchBuildingSitePositionsForTile", () => {
    var testFetchBuildingSitePositionsForTile = function (tile) {
      ...
      var roomManager = new RoomManager(room);
      var positions = roomManager.fetchBuildingSitePositionsForTile(
        tile.expectedCharacter,
      );

      assert.notEqual(
        undefined,
        positions,
        "Positions for " + JSON.stringify(tile.stuff) + " was undefined!",
      );
      assert.equal(
        1,
        positions.length,
        "Could not find " + JSON.stringify(tile.stuff) + " in room!",
      );
  ...
  describe("#fetchFreeBuildingSlots", () => {
    // fetchAvailableSlots & fetchUsedSlots
    var testFetchFreeBuildingSlots = function (
      tileCharacter,
      roomLevel,
      usedStructures,
      expectedResult,
      ...
      room.find = () => {
        var tile = allTiles.filter(
          (tile) => tile.expectedCharacter == tileCharacter,
        )[0];
        var result = [];
      ...
      assert.equal(
        expectedResult,
        result,
        "Unexpected result for fetchFreeBuildingSlots()! roomLevel=" +
          roomLevel +
          ", usedStructures=" +
          usedStructures,
      );
  ...
  describe("#generateLayout", () => {
    ...
    it("with array", () => {
      ...
      var room = new Room();
      room.lookAt = (x, y) =>
        y == startY && x < allTiles.length ? [allTiles[x + 3].stuff] : [];
    ...
    it("with spawn", () => {
      ...
      var compactString = "o o o" + " S o " + "o o o";
