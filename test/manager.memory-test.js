...
describe("manager.memory", () => {
  ...
  it("exists", () => {
    ...
    assert.equal(
      startsWith,
      MemoryManager.toString().substring(0, startsWith.length),
    );
  });
  ...
  describe("#getRequiredNumberForRoomAndRole", () => {
    it("not defined no base", () => {
      ...
      assert.equal(
        -1,
        MemoryManager.getRequiredNumberForRoomAndRole(room, role.roleName),
      );
    });
  ...
  describe("#fetchRoomRoleConfigForBase", () => {
    it("found base", () => {
      ...
      var roleConfig = MemoryManager._fetchRoomRoleConfig(room2, [
        { roleName: "Role" },
      ]);
      ...
      assert.deepEqual(
        roleConfig,
        MemoryManager.fetchRoomRoleConfigForBase("New York"),
      );
    });
