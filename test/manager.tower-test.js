...
describe("manager.base", () => {
  it("exists", () => {
    ...
    assert.equal(
      startsWith,
      TowerManager.toString().substring(0, startsWith.length),
    );
  });
  ...
  describe("#runTower", () => {
    it("do nothing", () => {
      ...
      tower.attack = (target) =>
        assert.fail("attack() should not have been called");
      tower.repair = (target) =>
        assert.fail("repair() should not have been called");
    ...
    it("attack", () => {
      ...
      tower.repair = (target) =>
        assert.fail("repair() should not have been called");
      tower.pos.findClosestByRange = (type) =>
        type == FIND_HOSTILE_CREEPS ? "creep" : null;
    ...
    it("repair", () => {
      ...
      tower.attack = (target) =>
        assert.fail("attack() should not have been called");
      tower.pos.findClosestByRange = (type) =>
        type == FIND_MY_CREEPS ? "creep" : null;
