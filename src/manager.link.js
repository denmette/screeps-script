...
class LinkManager {
  ...
  _runLink(sourceLink) {
    ...
    if (targetLink) {
      ...
      if (
        MainUtil.fetchMemoryOfStructure(sourceLink).debug ||
        MainUtil.fetchMemoryOfStructure(targetLink).debug
      ) {
        info.log(
          "💫 transfering resources from " +
            MainUtil.getDisplayName(sourceLink) +
            " to " +
            MainUtil.getDisplayName(targetLink),
          this._room,
        );
      }
    }
  ...
  _findTargetLink(sourceLink) {
    ...
    var targetLinks = this._room.find(FIND_MY_STRUCTURES, {
      filter: (structure) => {
        ...
        return (
          MainUtil.fetchMemoryOfStructure(structure).type == TYPE_TARGET &&
          structure.store.getFreeCapacity(RESOURCE_ENERGY) > 0
        );
      },
    });
