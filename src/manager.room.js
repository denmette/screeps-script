...
class RoomManager {
  ...
  _generateStructuresToBeBuild(array) {
    ...
    if (spawns.length > 0) {
      ...
      var extensionPositions = this._findExtensionPointsForSpawn(
        array,
        spawn,
        extensionsCount,
      );
      extensionPositions.forEach((pos) =>
        array.set(pos.x, pos.y, RoomManager.TILE_EXTENSION),
      );
    }
  }
  ...
  _findExtensionPointsForSpawn(
    array,
    spawn,
    limit = array.width * array.height,
  ) {
  ...
  _fetchMemoryOfManager() {
    ...
    if (this._room.memory.roomManager) {
      this._room.memory.roomManager = Object.assign(
        defaultArray,
        this._room.memory.roomManager,
      );
    } else {
  ...
  _fetchTileAt(x, y) {
    ...
    var endless = this._filterByTiles(
      stuffs,
  ...
  fetchFreeBuildingSlots(tileCharacter) {
    var tiles = RoomManager.TILES_TO_BE_BUILD.filter(
      (tile) => tile.character == tileCharacter,
    );
    if (tiles.length == 0) {
      MainInfo.error(
        'Could not find building for character "' + tileCharacter + '"!',
      );
      return;
    }
  ...
  _fetchUsedSlots(structureType) {
    return this._room.find(FIND_STRUCTURES, {
  ...
  _printRoom(array) {
    for (var x = 0; x < array.width; x++) {
      for (var y = 0; y < array.height; y++) {
        ...
        if (value !== undefined) {
          this._room.visual.text(value, x, y, {
            align: "center",
            opacity: 0.8,
          });
        }
...
class RoomPresetTile {
  ...
  isEqual(stuff) {
    return (
      stuff.type === this._type &&
      (!this._structureType ||
        (stuff.structure &&
          stuff.structure.structureType == this._structureType))
    );
  }
}
...
class RoomStructureTile {
  ...
  isEqual(stuff) {
    return (
      stuff.type === LOOK_STRUCTURES &&
      stuff.structure &&
      stuff.structure.structureType == this.structureType
    );
  }
}
