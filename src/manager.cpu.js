...
class GcManager {
  ...
  static visualize() {
    MainUtil.findAllRooms().forEach((room) =>
      this._visualizeMeasurements(room),
    );
  }
  ...
  static _visualizeMeasurements(room) {
    ...
    room.visual.text("CPU Watcher", x, y, {
      align: "left",
      opacity: memory.opacity + 0.2,
    });
    y++;

    this._measurements.forEach((measurement) => {
      var color = measurement.usedTicks > 10 ? "red" : "white";
      room.visual.text(measurement.name, x, y, {
        align: "left",
        opacity: memory.opacity,
        color: color,
      });
      room.visual.text(
        this._format.format(measurement.usedTicks),
        x + spaceToUsedTicks,
        y,
        { align: "right", opacity: memory.opacity, color: color },
      );
      y++;
    });
  }
...
GcManager._format = new Intl.NumberFormat("en", {
