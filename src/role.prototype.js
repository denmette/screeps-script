...
class RolePrototype {
  ...
  _moveToClosestSource(creep) {
    ...
    var harvestResult = source.structureType
      ...
      if (creep.memory.debug) {
        info.log(
          this.symbol +
            ...
            MainUtil.getDisplayName(creep) +
            " is moving to source " +
            MainUtil.getDisplayName(source),
          this._baseRoom,
        );
      }
  ...
  _findSources(room) {
            ...
            return (
              (structure.structureType == STRUCTURE_STORAGE ||
                ...
                (structure.structureType == STRUCTURE_LINK &&
                  structure.memory &&
                  structure.memory.type == constants.LINK_TYPE_TARGET)) &&
    ...
    var sources = this._useSourceAsSource
      ? room.find(FIND_SOURCES_ACTIVE, {
          filter: (source) => {
            if (
              MainUtil.findAllCreeps().filter(
                (creep) =>
  ...
  run(creep, baseRoom = RolePrototype._fetchBaseRoomForCreep(creep)) {
    ...
    var memoryRoleConfig = MemoryManager.fetchRoomRoleConfigForBase(
      creep.memory.home,
      [this],
    );
    this._useStorageAsSource =
      !memoryRoleConfig || memoryRoleConfig.useStorageAsSource;
    this._useSourceAsSource =
      !memoryRoleConfig || memoryRoleConfig.useSourceAsSource;
    ...
    if (droppedEnergy.length > 0 && creep.store.getFreeCapacity() > 0) {
      if (creep.memory.debug) {
        info.log(
          "🟡 " +
            MainUtil.getDisplayName(creep) +
            " is picking up resource " +
            MainUtil.getDisplayName(droppedEnergy[0]),
          this._baseRoom,
        );
      }
  ...
  _selfdestruct(creep) {
    ...
    if (spawn) {
      ...
      if (recycleAnswer == ERR_NOT_IN_RANGE) {
        ...
        if (creep.memory.debug) {
          info.log(
            "🗑 " + MainUtil.getDisplayName(creep) + " was recycled.",
            this._baseRoom,
          );
        }
      }
      ...
      info.error(
        MainUtil.getDisplayName(creep) + " could not find a spawn.",
        this._baseRoom,
      );
    }
  }
  ...
  static _fetchBaseRoomForName(baseName) {
    ...
    var baseRooms = allRooms
      ? allRooms.filter((room) =>
          room.memory.base ? room.memory.base.name == baseName : false,
        )
      : [];
    ...
    if (withdrawResult == ERR_NOT_IN_RANGE) {
      if (creep.memory.debug) {
        info.log(
          "⚰ " +
            MainUtil.getDisplayName(creep) +
            " is moving to tombstone " +
            MainUtil.getDisplayName(tombstone),
          this._baseRoom,
        );
      }
  ...
  _spawnCreepWithParts(spawn, parts, singleParts = []) {
    ...
    if (parts) {
      ...
      var spawnResult = spawn.spawnCreep(parts, newName, {
        memory: {
          role: this.roleName,
  ...
  _calculateMaxParts(spawn, parts = [], singleParts = []) {
    ...
    var singleCosts = singleParts
      ? this._calculateCostsForParts(singleParts)
      : 0;
    ...
    while (
      (multiplier + 1) * costs + singleCosts <= spawn.room.energyAvailable &&
      multiplier < partsMaxMultiplier
    ) {
      multiplier++;
    }
    ...
    return singleParts
      ? singleParts.concat(this._replicateParts(parts, multiplier))
  ...
  _getPartsMinMultiplier(spawn) {
    return (
      (spawn.room.memory.base &&
        ...
        spawn.room.memory.base.roleConfig.partsMinMultiplier) ||
      0
    );
  }
  ...
  _getPartsMaxMultiplier(spawn) {
    return (
      (spawn.room.memory.base &&
        spawn.room.memory.base.roleConfig &&
        spawn.room.memory.base.roleConfig.partsMaxMultiplier) ||
      5
    );
  }
  ...
  _calculateCostsForParts(parts) {
    var result = 0;
    parts.forEach(
      (part) => (result += this._calculateCostsForSinglePart(part)),
    );
    return result;
  }
