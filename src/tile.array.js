...
class TileArray {
  ...
  toCompactString(opt = {}) {
    ...
    var maxLength = Math.max(
      ...flatMap.map((value) => (value ? ("" + value).length : 0)),
    );
    return flatMap
      .map((value) =>
        value
          ? ("" + value).padStart(maxLength, opt.paddingChar)
          : opt.paddingChar.repeat(maxLength),
      )
      .join("");
  }
    ...
    for (var y = 0; y < this.height; y++) {
      for (var x = 0; x < this.width; x++) {
        ...
        this.set(
          ...
          y,
          string
            .substring(position, position + valueLength)
            .split(opt.paddingChar)
            .join(""),
        );
      }
    }
