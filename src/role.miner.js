...
class Miner extends RolePrototype {
  ...
  spawnCreep(spawn) {
    var claimedSources = MainUtil.findAllCreeps()
      ...
      .map((creep) => creep.memory.source);
    var allSources = spawn.room
      .find(FIND_SOURCES)
      .filter((source) => !claimedSources.includes(source.id));
  ...
  _work(creep) {
    ...
    if (creep.memory.ticksToSource && !creep.memory.trainedReplacement) {
      ...
      if (creep.ticksToLive < ticksToGetReady) {
        var spawnedCreep = this.spawnCreepFromSpawnName(
          creep.memory.homeSpawn,
          creep.memory.source,
        );
        if (spawnedCreep) {
          info.log(
            this.symbol + " Training the replacement " + this.roleName,
            this._baseRoom,
          );
    ...
    return room.find(FIND_STRUCTURES, {
      filter: (structure) => {
        return (
          ...
          (!testCreepPosition ||
            this.creep.pos.inRangeTo(structure, this._maxRangeToTarget)) &&
