...
class BaseManager {
  ...
  _repopulateCreeps() {
    ...
    this._allRoles.forEach((role) => {
      ...
      var foundCreeps = MainUtil.findAllCreeps().filter(
        (creep) =>
  ...
  _fetchFreeSpawn(baseName) {
    var freeSpawns = MainUtil.findAllSpawns().filter(
      (spawn) =>
        (!baseName || spawn.memory.home == baseName) && !spawn.spawning,
    );
  ...
  _showSpawningAnimation() {
    MainUtil.findAllSpawns()
      ...
      .forEach((spawn) => {
        spawn.room.visual.text("🔁", spawn.pos.x - 1, spawn.pos.y, {
          align: "left",
          opacity: 0.8,
        });
      });
    ...
    MainUtil.findAllCreeps()
      ...
      .forEach((creep) => {
        try {
          ...
          var currentNumber =
            creep.room.memory.roleInfo[creepRole.roleName].currentNumber;
          creep.room.memory.roleInfo[creepRole.roleName].currentNumber =
            currentNumber ? currentNumber + 1 : 1;

          if (
            this._room.memory.base.roleConfig.showIcons &&
            this._room == creep.room
          ) {
            this._room.visual.text(creepRole.symbol, creep.pos.x, creep.pos.y, {
              align: "left",
              opacity: 0.8,
            });
          }
  ...
  _findNecessaryMandatoryRole(roleName) {
    ...
    if (!result.isNecessary(this._room)) {
      ...
      var necessaryRoles = this._allRoles.filter((role) =>
        role.isNecessary(this._room),
      );
