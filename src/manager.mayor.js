...
class MayorManager {
  ...
  static runAll() {
    MainUtil.findAllRooms().forEach((room) => {
      try {
        ...
        info.error(
          "Could not run Mayor for room " +
            MainUtil.getDisplayName(room) +
            ": " +
            e,
        );
        console.log(e.stack);
  ...
  _fetchMemoryOfMayor() {
    ...
    if (this._room.memory.mayor) {
      this._room.memory.mayor = Object.assign(
        defaultArray,
        this._room.memory.mayor,
      );
    } else {
      ...
      .filter((pos) => {
        if (
          pos
            ...
            .filter(
              (obj) =>
                obj.structure && obj.structure.structureType == STRUCTURE_ROAD,
            ).length > 0
        ) {
  ...
  _buildRoads(roadPositions) {
    ...
    var roadConstructionSites = this._room.find(FIND_CONSTRUCTION_SITES, {
  ...
  _validateCreeps() {
    ...
    if (this._room.memory.base.roleConfig.Upgrader.requiredNumber == 0) {
      this.errors.push({
        ...
        act: () =>
          (this._room.memory.base.roleConfig.Upgrader.requiredNumber = 1),
      });
      return;
    }
    if (
      this._room.find(FIND_CONSTRUCTION_SITES).length > 0 &&
      this._room.memory.base.roleConfig.Builder.requiredNumber == 0
    ) {
      this.errors.push({
        text: "Builder is missing.",
        solution: "Spawning Builder.",
        act: () =>
          (this._room.memory.base.roleConfig.Builder.requiredNumber = 1),
      });
      return;
    }
    if (
      this._room.find(FIND_STRUCTURES, { filter: (s) => s.hits < s.hitsMax })
        .length > 0 &&
      this._room.memory.base.roleConfig.Handyman.requiredNumber == 0
    ) {
      this.errors.push({
        text: "Handyman is missing.",
        solution: "Spawning Handyman.",
        act: () =>
          (this._room.memory.base.roleConfig.Handyman.requiredNumber = 1),
      });
      return;
    }
  }
  ...
  _validateBuildings() {
    ...
    var notRoadConstructionSites = this._room.find(FIND_CONSTRUCTION_SITES, {
      filter: (site) => site.structureType != STRUCTURE_ROAD,
    });
    ...
    if (!somethingWasBuilt) {
      for (var i = 0; i < RoomManager.TILES_ENDLESS_STRUCTURES.length; i++) {
        ...
        var buildingPositions = this._fetchBuildingSitePositionsForTile(
          roomManager,
          tileToBeBuild,
        );
        ...
        if (buildingPositions.length > 0) {
          this.warnings.push({
            ...
            solution: "I'm building " + tileToBeBuild.structureType,
            act: () =>
              this._buildStructureAtPosition(
                buildingPositions[0],
                tileToBeBuild,
              ),
          });
  ...
  _buildStructure(roomManager, tileToBeBuild) {
    ...
    if (buildingPositions.length > 0) {
      ...
      info.warning(
        "Could not find building position for " + tileToBeBuild.structureType,
      );
    }
  }
  ...
  _fetchBuildingSitePositionsForTile(roomManager, tileToBeBuild) {
    ...
    return roomManager
      .fetchBuildingSitePositionsForTile(tileToBeBuild.character)
      .filter(
        (pos) =>
          pos.look().filter((obj) => obj.type == LOOK_STRUCTURES).length == 0,
      )
      .sort(
        (a, b) =>
  ...
  _buildStructureAtPosition(buildingPosition, tileToBeBuild) {
    var constructionResult = buildingPosition.createConstructionSite(
      tileToBeBuild.structureType,
    );
  ...
  _visualize() {
    ...
    this._room.visual.text(
      "👷‍♀️ " + this._getModeDisplayName(this._memory.mode),
      x,
      y++,
    ...
    this.errors.forEach((error) => {
      this._room.visual.text(
        "🔴 " + (warn ? error.text : error.solution),
        x + 1,
        y++,
    ...
    if (this.errors.length <= 3) {
      this.warnings.forEach((warning) => {
        this._room.visual.text(
          "🟡 " + (warn ? warning.text : warning.solution),
          x + 1,
          y++,
...
MayorManager.MODES_ALL = [
  MayorManager.MODE_OFF,
  MayorManager.MODE_WARN,
  MayorManager.MODE_AUTO,
];
