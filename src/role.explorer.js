...
class Explorer extends RolePrototype {
  ...
  spawnCreep(spawn) {
    var claimedFlags = MainUtil.findAllCreeps()
      .filter((creep) => creep.memory.target)
      .map((creep) => creep.memory.target);
    var allFlags = MainUtil.findAllFlags().filter(
      (flag) => !claimedFlags.includes(flag.name),
    );
  ...
  spawnCreepFromSpawnName(spawnName, flagName) {
    var spawns = MainUtil.findAllSpawns().filter(
      (spawn) => spawn.name == spawnName,
    );
    if (spawns.length == 0) {
  ...
  _spawnCreepFromSpawn(spawn, flagName) {
    var resultingCreep = this._spawnCreepWithParts(
      spawn,
  ...
  _claimFlagRoom(creep) {
    var targetFlag = this._findTargets(creep.room).filter(
      (target) => target.name == creep.memory.target,
    )[0];
    ...
    if (creep.room.controller.my) {
      ...
      creep.memory.phase = hasSpawnAlready
        ? PHASE_BUILD_SPAWN
        : PHASE_CREATE_SPAWN;
      info.log(
        this.symbol +
  ...
  _findSpawns(room) {
    if (!room) {
      room = this._findTargets(creep.room).filter(
        (target) => target.name == creep.memory.target,
      )[0].room;
    }
  ...
  _createSpawn(creep) {
    ...
    var hasSpawnConstructionSiteAlready =
    ...
    var createResult = targetFlag.pos.createConstructionSite(
      STRUCTURE_SPAWN,
      targetFlag.name,
    );
    if (createResult == OK) {
