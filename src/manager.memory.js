...
class MemoryManager {
  ...
  static _fetchRoomRoleConfig(room, allRoles = []) {
    ...
    if (room.memory.base.roleConfig) {
      room.memory.base.roleConfig = Object.assign(
        defaultArray,
  ...
  static _clearRoomRoleInfo(room, allRoles) {
    ...
    allRoles.forEach((role) => {
      defaultArray[role.roleName] = {
        symbol: role.symbol,
        currentNumber: 0,
        requiredNumber: this.getRequiredNumberForRoomAndRole(
          room,
          role.roleName,
        ),
      };
  ...
  static getRequiredNumberForRoomAndRole(room, roleName) {
    ...
    return hasBase
      ? room.memory.base.roleConfig && room.memory.base.roleConfig[roleName]
        ? room.memory.base.roleConfig[roleName].requiredNumber
        : 0
      : -1;
  }
  ...
  static _initSpawns() {
    ...
    MainUtil.findAllSpawns()
      .filter((spawn) => !spawn.memory.home)
      .forEach((spawn) => {
